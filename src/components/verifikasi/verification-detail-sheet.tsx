...
import TolakDialog from './TolakDialog'
...
const initialDetailArsip: ArsipItem[] = [
  { id: 1, nomorHak: 'HGB 123456', kecamatan: 'Pademangan',   desa: 'Ancol',        keperluan: 'Sengketa',       status: 'PENGAJUAN', keteranganTolak: '' },
  { id: 2, nomorHak: 'HGB 654321', kecamatan: 'Tanjung Priok',desa: 'Sunter',       keperluan: 'Pengukuran',     status: 'PENGAJUAN', keteranganTolak: '' },
  { id: 3, nomorHak: 'HGB 777888', kecamatan: 'Koja',         desa: 'Koja Selatan', keperluan: 'Permohonan Hak', status: 'PENGAJUAN', keteranganTolak: '' },
]
...
export default function VerificationDetailSheet({ open, onOpenChange, data }: Props) {
  ...
  const setujui = (id: number) => {
    setDetailArsip((prev) => prev.map((item) => item.id === id ? { ...item, status: 'DISETUJUI' as StatusType, keteranganTolak: '' } : item))
  }
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          <SheetHeader className="shrink-0 border-b border-zinc-100 dark:border-zinc-800 px-4 pt-4 pb-5 lg:px-6">
            <SheetTitle className="text-zinc-900 dark:text-white">Detail Peminjaman Arsip</SheetTitle>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 lg:px-4 lg:py-6">
            <div className="overflow-hidden rounded-[28px] bg-white dark:bg-zinc-800/60 transition-colors duration-300">
              <div className="border-b border-zinc-100 dark:border-zinc-700 px-6 py-5">
                ...
                <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Total {detailArsip.length} arsip yang diajukan</p>
              </div>
