...
export default function VerificationTable({ onDetail }: Props) {
  return (
    <div className="overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 transition-colors duration-300 sm:rounded-[32px]">
      <div className="border-b border-zinc-100 dark:border-zinc-800 px-4 py-4 sm:px-6 sm:py-5">
        <h2 className="text-base font-semibold text-zinc-900 dark:text-white sm:text-lg">Daftar Pengajuan</h2>
        <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400 sm:text-sm">Menampilkan {data.length} data peminjaman</p>
      </div>
        ...
        {data.map((item) => (
          <div key={item.id} className="rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4 transition-colors duration-200">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                <p className="text-xs text-zinc-500 dark:text-zinc-400">Tanggal Pengajuan</p>
