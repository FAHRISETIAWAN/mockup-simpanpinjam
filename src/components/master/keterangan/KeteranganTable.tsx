...
import KeteranganDialog from './KeteranganDialog'

const initialData = [
  { id: 1, kode: 'KET-001', keterangan: 'Aktif', tanggal: '04 Juni 2026' },
  { id: 2, kode: 'KET-002', keterangan: 'Dalam Proses', tanggal: '04 Juni 2026' },
  { id: 3, kode: 'KET-003', keterangan: 'Selesai', tanggal: '04 Juni 2026' },
]

export default function KeteranganTable() {
  const [data, setData] = useState(initialData)
  const [open, setOpen] = useState(false)
  const [selected, setSelected] = useState<any>(null)

  const handleAdd = (kode: string, keterangan: string) => {
    setData((prev) => [
      {
        id: Date.now(),
        kode,
  ...
  const handleEdit = (kode: string, keterangan: string) => {
    setData((prev) =>
      prev.map((item) =>
        item.id === selected.id ? { ...item, kode, keterangan } : item
      )
    )
  }
  ...
  return (
    <>
      <div className="overflow-hidden rounded-[32px] bg-white dark:bg-zinc-900 transition-colors duration-300">
        ...
        <div className="flex flex-col gap-4 border-b border-zinc-100 dark:border-zinc-800 px-6 py-5 md:flex-row md:items-center md:justify-between">
          ...
          <Button
            ...
            className="h-11 rounded-full bg-orange-500 cursor-pointer px-5 shadow-md shadow-orange-200 dark:shadow-orange-900/30 hover:bg-orange-600 transition-all duration-200 hover:scale-105 active:scale-95"
          >
          ...
          <table className="w-full">
            ...
            <tbody>
              {data.map((item) => (
                <tr
                  key={item.id}
                  className="border-b border-zinc-100 dark:border-zinc-800 transition-all duration-200 hover:bg-orange-50/20 dark:hover:bg-orange-950/10 last:border-0"
                >
                  <td className="px-6 py-4">
                    <span className="inline-flex rounded-full bg-violet-100 dark:bg-violet-900/40 px-3 py-1 text-xs font-semibold text-violet-700 dark:text-violet-400">
                      {item.kode}
                    </span>
                  </td>
          ...
          {data.map((item) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-xs text-zinc-400 dark:text-zinc-500">{item.tanggal}</span>
              </div>

              <p className="mt-3 font-medium text-zinc-900 dark:text-white">{item.keterangan}</p>
      ...
      <KeteranganDialog
        ...
        initialKode={selected?.kode}
        initialKeterangan={selected?.keterangan}
        onSubmit={(kode, keterangan) => {
          if (selected) handleEdit(kode, keterangan)
          else handleAdd(kode, keterangan)
        }}
      />
    </>
  )
